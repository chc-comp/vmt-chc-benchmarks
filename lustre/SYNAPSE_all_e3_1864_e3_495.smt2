;; translated from ./lustre/SYNAPSE_all_e3_1864_e3_495.vmt
;; original benchmark available at: https://es-static.fbk.eu/people/griggio/ic3ia/index.html
;; author: Alberto Griggio <griggio@fbk.eu>
(set-logic HORN)
(declare-fun state (Int Int Bool Bool Bool Bool Bool Bool Int Int Int Int Int Int Int Int Int Int Int Int Int Int Int Int Int Int Int Int Int Int Int Bool Int Int Int Int Bool Bool Int Int Bool Int Bool Bool Int) Bool)
(assert (forall ((|call_synapse.garde_s1__AT0| Bool) (|call_synapse.xite13__AT0| Int) (|call_synapse.xite14__AT0| Int) (|call_synapse.garde_s2__AT0| Bool) (|call_synapse.xite8__AT0| Int) (|call_synapse.garde_s3__AT0| Bool) (|param_init_invalid_s__AT0| Int) (|call_synapse.flby__AT0| Int) (|call_synapse.flby2__AT0| Int) (|call_synapse.mem_init_s__AT0| Int) (|call_synapse.flby3__AT0| Int) (|call_synapse.flby4__AT0| Int) (|call_excludes3.excludes__AT0| Bool) (|call_Sofar.flby__AT0| Bool) (|call_First.flby__AT0| Int) (|_OK___AT0| Bool) (|env__AT0| Bool) (|dirty_s__AT0| Int) (|valid_s__AT0| Int) (|invalid_s__AT0| Int) (|call_synapse.dirty_s__AT0| Int) (|mem_init_s__AT0| Int) (|call_Sofar.Sofar__AT0| Bool) (|call_synapse.invalid_s__AT0| Int) (|call_synapse.xite6__AT0| Int) (|call_synapse.valid_s__AT0| Int) (|call_synapse.xite5__AT0| Int) (|call_First.First__AT0| Int) (|param_e_s1__AT0| Bool) (|call_synapse.xite__AT0| Int) (|call_synapse.xite7__AT0| Int) (|call_synapse.xite12__AT0| Int) (|call_synapse.xite11__AT0| Int) (|call_synapse.xite18__AT0| Int) (|call_synapse.xite17__AT0| Int) (|param_e_s2__AT0| Bool) (|call_synapse.xite2__AT0| Int) (|call_synapse.xite4__AT1| Int) (|param_e_s3__AT1| Bool) (|call_synapse.xite3__AT1| Int) (|call_synapse.xite9__AT1| Int) (|call_synapse.xite10__AT1| Int) (|call_synapse.xite15__AT1| Int) (|call_synapse.xite16__AT1| Int) (|invalid_s__AT1| Int) (|valid_s__AT1| Int) (|dirty_s__AT1| Int) (|flby__AT1| Bool) (|call_synapse.xite__AT1| Int) (|call_synapse.garde_s1__AT1| Bool) (|call_synapse.xite2__AT1| Int) (|call_synapse.garde_s2__AT1| Bool) (|call_synapse.garde_s3__AT1| Bool) (|call_synapse.xite7__AT1| Int) (|call_synapse.xite8__AT1| Int) (|call_synapse.xite14__AT1| Int) (|call_synapse.xite13__AT1| Int) (|call_synapse.flby__AT1| Int) (|call_synapse.xite6__AT1| Int) (|call_synapse.flby2__AT1| Int) (|call_synapse.xite12__AT1| Int) (|call_synapse.flby3__AT1| Int) (|call_synapse.xite18__AT1| Int) (|call_synapse.flby4__AT1| Int) (|param_init_invalid_s__AT1| Int) (|call_excludes3.excludes__AT1| Bool) (|call_Sofar.flby__AT1| Bool) (|call_First.flby__AT1| Int) (|_OK___AT1| Bool) (|env__AT1| Bool) (|call_First.First__AT1| Int) (|call_synapse.invalid_s__AT1| Int) (|call_synapse.valid_s__AT1| Int) (|call_synapse.dirty_s__AT1| Int) (|mem_init_s__AT1| Int) (|call_synapse.mem_init_s__AT1| Int) (|call_Sofar.Sofar__AT1| Bool) (|call_synapse.xite5__AT1| Int) (|param_e_s1__AT1| Bool) (|call_synapse.xite11__AT1| Int) (|call_synapse.xite17__AT1| Int) (|param_e_s2__AT1| Bool) (|call_synapse.xite9__AT0| Int) (|call_synapse.xite10__AT0| Int) (|param_e_s3__AT0| Bool) (|call_synapse.xite3__AT0| Int) (|call_synapse.xite4__AT0| Int) (|call_synapse.xite15__AT0| Int) (|call_synapse.xite16__AT0| Int) (|flby__AT0| Bool)) (=> (let ((.def_178 (= call_synapse.xite10__AT0 call_synapse.xite9__AT0)))
(let ((.def_173 (not param_e_s3__AT0)))
(let ((.def_179 (or .def_173 .def_178)))
(let ((.def_176 (= call_synapse.xite4__AT0 call_synapse.xite3__AT0)))
(let ((.def_177 (or .def_173 .def_176)))
(let ((.def_180 (and .def_177 .def_179)))
(let ((.def_172 (= call_synapse.xite16__AT0 call_synapse.xite15__AT0)))
(let ((.def_174 (or .def_172 .def_173)))
(let ((.def_181 (and .def_174 .def_180)))
(let ((.def_182 (and flby__AT0 .def_181)))
(let ((.def_170 (not call_synapse.garde_s1__AT0)))
(let ((.def_168 (= call_synapse.xite13__AT0 0)))
(let ((.def_171 (or .def_168 .def_170)))
(let ((.def_183 (and .def_171 .def_182)))
(let ((.def_165 (= call_synapse.xite14__AT0 1)))
(let ((.def_163 (not call_synapse.garde_s2__AT0)))
(let ((.def_166 (or .def_163 .def_165)))
(let ((.def_161 (= call_synapse.xite8__AT0 0)))
(let ((.def_164 (or .def_161 .def_163)))
(let ((.def_167 (and .def_164 .def_166)))
(let ((.def_184 (and .def_167 .def_183)))
(let ((.def_158 (= call_synapse.xite15__AT0 1)))
(let ((.def_155 (not call_synapse.garde_s3__AT0)))
(let ((.def_159 (or .def_155 .def_158)))
(let ((.def_153 (= call_synapse.xite9__AT0 0)))
(let ((.def_156 (or .def_153 .def_155)))
(let ((.def_160 (and .def_156 .def_159)))
(let ((.def_185 (and .def_160 .def_184)))
(let ((.def_151 (= call_synapse.flby__AT0 param_init_invalid_s__AT0)))
(let ((.def_186 (and .def_151 .def_185)))
(let ((.def_150 (= call_synapse.mem_init_s__AT0 call_synapse.flby2__AT0)))
(let ((.def_187 (and .def_150 .def_186)))
(let ((.def_149 (= call_synapse.flby3__AT0 0)))
(let ((.def_188 (and .def_149 .def_187)))
(let ((.def_148 (= call_synapse.flby4__AT0 0)))
(let ((.def_189 (and .def_148 .def_188)))
(let ((.def_145 (<= 0 param_init_invalid_s__AT0)))
(let ((.def_146 (and call_excludes3.excludes__AT0 .def_145)))
(let ((.def_147 (= call_Sofar.flby__AT0 .def_146)))
(let ((.def_190 (and .def_147 .def_189)))
(let ((.def_144 (= call_First.flby__AT0 param_init_invalid_s__AT0)))
(let ((.def_191 (and .def_144 .def_190)))
(let ((.def_125 (not env__AT0)))
(let ((.def_121 (<= 1 dirty_s__AT0)))
(let ((.def_122 (not .def_121)))
(let ((.def_119 (<= 1 valid_s__AT0)))
(let ((.def_120 (not .def_119)))
(let ((.def_123 (or .def_120 .def_122)))
(let ((.def_111 (* (- 1) invalid_s__AT0)))
(let ((.def_112 (* (- 1) valid_s__AT0)))
(let ((.def_113 (+ .def_112 .def_111)))
(let ((.def_114 (* (- 1) dirty_s__AT0)))
(let ((.def_115 (+ .def_114 .def_113)))
(let ((.def_116 (+ call_First.First__AT0 .def_115)))
(let ((.def_117 (= .def_116 0)))
(let ((.def_100 (<= 2 dirty_s__AT0)))
(let ((.def_101 (not .def_100)))
(let ((.def_103 (and .def_101 flby__AT0)))
(let ((.def_118 (and .def_103 .def_117)))
(let ((.def_124 (and .def_118 .def_123)))
(let ((.def_126 (or .def_124 .def_125)))
(let ((.def_128 (= .def_126 _OK___AT0)))
(let ((.def_98 (= call_synapse.invalid_s__AT0 invalid_s__AT0)))
(let ((.def_129 (and .def_98 .def_128)))
(let ((.def_96 (= call_synapse.valid_s__AT0 valid_s__AT0)))
(let ((.def_130 (and .def_96 .def_129)))
(let ((.def_94 (= call_synapse.dirty_s__AT0 dirty_s__AT0)))
(let ((.def_131 (and .def_94 .def_130)))
(let ((.def_92 (= call_synapse.mem_init_s__AT0 mem_init_s__AT0)))
(let ((.def_132 (and .def_92 .def_131)))
(let ((.def_90 (= call_Sofar.Sofar__AT0 env__AT0)))
(let ((.def_133 (and .def_90 .def_132)))
(let ((.def_82 (= call_synapse.xite5__AT0 call_synapse.xite6__AT0)))
(let ((.def_83 (or param_e_s1__AT0 .def_82)))
(let ((.def_80 (= call_synapse.xite6__AT0 call_synapse.xite__AT0)))
(let ((.def_70 (not param_e_s1__AT0)))
(let ((.def_81 (or .def_70 .def_80)))
(let ((.def_84 (and .def_81 .def_83)))
(let ((.def_76 (= call_synapse.xite12__AT0 call_synapse.xite7__AT0)))
(let ((.def_77 (or .def_70 .def_76)))
(let ((.def_85 (and .def_77 .def_84)))
(let ((.def_73 (= call_synapse.xite11__AT0 call_synapse.xite12__AT0)))
(let ((.def_74 (or param_e_s1__AT0 .def_73)))
(let ((.def_86 (and .def_74 .def_85)))
(let ((.def_69 (= call_synapse.xite18__AT0 call_synapse.xite13__AT0)))
(let ((.def_71 (or .def_69 .def_70)))
(let ((.def_87 (and .def_71 .def_86)))
(let ((.def_66 (= call_synapse.xite17__AT0 call_synapse.xite18__AT0)))
(let ((.def_67 (or param_e_s1__AT0 .def_66)))
(let ((.def_88 (and .def_67 .def_87)))
(let ((.def_134 (and .def_88 .def_133)))
(let ((.def_58 (= call_synapse.xite5__AT0 call_synapse.xite4__AT0)))
(let ((.def_59 (or param_e_s2__AT0 .def_58)))
(let ((.def_55 (= call_synapse.xite5__AT0 call_synapse.xite2__AT0)))
(let ((.def_44 (not param_e_s2__AT0)))
(let ((.def_56 (or .def_44 .def_55)))
(let ((.def_60 (and .def_56 .def_59)))
(let ((.def_51 (= call_synapse.xite11__AT0 call_synapse.xite8__AT0)))
(let ((.def_52 (or .def_44 .def_51)))
(let ((.def_61 (and .def_52 .def_60)))
(let ((.def_48 (= call_synapse.xite11__AT0 call_synapse.xite10__AT0)))
(let ((.def_49 (or param_e_s2__AT0 .def_48)))
(let ((.def_62 (and .def_49 .def_61)))
(let ((.def_43 (= call_synapse.xite17__AT0 call_synapse.xite14__AT0)))
(let ((.def_45 (or .def_43 .def_44)))
(let ((.def_63 (and .def_45 .def_62)))
(let ((.def_40 (= call_synapse.xite17__AT0 call_synapse.xite16__AT0)))
(let ((.def_41 (or param_e_s2__AT0 .def_40)))
(let ((.def_64 (and .def_41 .def_63)))
(let ((.def_135 (and .def_64 .def_134)))
(let ((.def_37 (= call_synapse.flby2__AT0 call_synapse.invalid_s__AT0)))
(let ((.def_136 (and .def_37 .def_135)))
(let ((.def_34 (= call_synapse.flby3__AT0 call_synapse.valid_s__AT0)))
(let ((.def_137 (and .def_34 .def_136)))
(let ((.def_31 (= call_synapse.flby4__AT0 call_synapse.dirty_s__AT0)))
(let ((.def_138 (and .def_31 .def_137)))
(let ((.def_28 (= call_synapse.flby__AT0 call_synapse.mem_init_s__AT0)))
(let ((.def_139 (and .def_28 .def_138)))
(let ((.def_20 (and param_e_s2__AT0 param_e_s1__AT0)))
(let ((.def_19 (and param_e_s3__AT0 param_e_s1__AT0)))
(let ((.def_21 (or .def_19 .def_20)))
(let ((.def_17 (and param_e_s3__AT0 param_e_s2__AT0)))
(let ((.def_22 (or .def_17 .def_21)))
(let ((.def_23 (not .def_22)))
(let ((.def_25 (= .def_23 call_excludes3.excludes__AT0)))
(let ((.def_140 (and .def_25 .def_139)))
(let ((.def_14 (= call_Sofar.flby__AT0 call_Sofar.Sofar__AT0)))
(let ((.def_141 (and .def_14 .def_140)))
(let ((.def_11 (= call_First.flby__AT0 call_First.First__AT0)))
(let ((.def_142 (and .def_11 .def_141)))
(let ((.def_192 (and .def_142 .def_191)))
.def_192))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))) (state |call_First.flby__AT0| |call_First.First__AT0| |call_Sofar.flby__AT0| |call_Sofar.Sofar__AT0| |param_e_s3__AT0| |param_e_s2__AT0| |param_e_s1__AT0| |call_excludes3.excludes__AT0| |call_synapse.flby__AT0| |call_synapse.mem_init_s__AT0| |call_synapse.flby4__AT0| |call_synapse.dirty_s__AT0| |call_synapse.flby3__AT0| |call_synapse.valid_s__AT0| |call_synapse.flby2__AT0| |call_synapse.invalid_s__AT0| |call_synapse.xite17__AT0| |call_synapse.xite16__AT0| |call_synapse.xite14__AT0| |call_synapse.xite11__AT0| |call_synapse.xite10__AT0| |call_synapse.xite8__AT0| |call_synapse.xite5__AT0| |call_synapse.xite2__AT0| |call_synapse.xite4__AT0| |call_synapse.xite18__AT0| |call_synapse.xite13__AT0| |call_synapse.xite12__AT0| |call_synapse.xite7__AT0| |call_synapse.xite6__AT0| |call_synapse.xite__AT0| |env__AT0| |mem_init_s__AT0| |dirty_s__AT0| |valid_s__AT0| |invalid_s__AT0| |flby__AT0| |_OK___AT0| |param_init_invalid_s__AT0| |call_synapse.xite9__AT0| |call_synapse.garde_s3__AT0| |call_synapse.xite15__AT0| |call_synapse.garde_s2__AT0| |call_synapse.garde_s1__AT0| |call_synapse.xite3__AT0|))))
(assert (forall ((|call_synapse.garde_s1__AT0| Bool) (|call_synapse.xite13__AT0| Int) (|call_synapse.xite14__AT0| Int) (|call_synapse.garde_s2__AT0| Bool) (|call_synapse.xite8__AT0| Int) (|call_synapse.garde_s3__AT0| Bool) (|param_init_invalid_s__AT0| Int) (|call_synapse.flby__AT0| Int) (|call_synapse.flby2__AT0| Int) (|call_synapse.mem_init_s__AT0| Int) (|call_synapse.flby3__AT0| Int) (|call_synapse.flby4__AT0| Int) (|call_excludes3.excludes__AT0| Bool) (|call_Sofar.flby__AT0| Bool) (|call_First.flby__AT0| Int) (|_OK___AT0| Bool) (|env__AT0| Bool) (|dirty_s__AT0| Int) (|valid_s__AT0| Int) (|invalid_s__AT0| Int) (|call_synapse.dirty_s__AT0| Int) (|mem_init_s__AT0| Int) (|call_Sofar.Sofar__AT0| Bool) (|call_synapse.invalid_s__AT0| Int) (|call_synapse.xite6__AT0| Int) (|call_synapse.valid_s__AT0| Int) (|call_synapse.xite5__AT0| Int) (|call_First.First__AT0| Int) (|param_e_s1__AT0| Bool) (|call_synapse.xite__AT0| Int) (|call_synapse.xite7__AT0| Int) (|call_synapse.xite12__AT0| Int) (|call_synapse.xite11__AT0| Int) (|call_synapse.xite18__AT0| Int) (|call_synapse.xite17__AT0| Int) (|param_e_s2__AT0| Bool) (|call_synapse.xite2__AT0| Int) (|call_synapse.xite4__AT1| Int) (|param_e_s3__AT1| Bool) (|call_synapse.xite3__AT1| Int) (|call_synapse.xite9__AT1| Int) (|call_synapse.xite10__AT1| Int) (|call_synapse.xite15__AT1| Int) (|call_synapse.xite16__AT1| Int) (|invalid_s__AT1| Int) (|valid_s__AT1| Int) (|dirty_s__AT1| Int) (|flby__AT1| Bool) (|call_synapse.xite__AT1| Int) (|call_synapse.garde_s1__AT1| Bool) (|call_synapse.xite2__AT1| Int) (|call_synapse.garde_s2__AT1| Bool) (|call_synapse.garde_s3__AT1| Bool) (|call_synapse.xite7__AT1| Int) (|call_synapse.xite8__AT1| Int) (|call_synapse.xite14__AT1| Int) (|call_synapse.xite13__AT1| Int) (|call_synapse.flby__AT1| Int) (|call_synapse.xite6__AT1| Int) (|call_synapse.flby2__AT1| Int) (|call_synapse.xite12__AT1| Int) (|call_synapse.flby3__AT1| Int) (|call_synapse.xite18__AT1| Int) (|call_synapse.flby4__AT1| Int) (|param_init_invalid_s__AT1| Int) (|call_excludes3.excludes__AT1| Bool) (|call_Sofar.flby__AT1| Bool) (|call_First.flby__AT1| Int) (|_OK___AT1| Bool) (|env__AT1| Bool) (|call_First.First__AT1| Int) (|call_synapse.invalid_s__AT1| Int) (|call_synapse.valid_s__AT1| Int) (|call_synapse.dirty_s__AT1| Int) (|mem_init_s__AT1| Int) (|call_synapse.mem_init_s__AT1| Int) (|call_Sofar.Sofar__AT1| Bool) (|call_synapse.xite5__AT1| Int) (|param_e_s1__AT1| Bool) (|call_synapse.xite11__AT1| Int) (|call_synapse.xite17__AT1| Int) (|param_e_s2__AT1| Bool) (|call_synapse.xite9__AT0| Int) (|call_synapse.xite10__AT0| Int) (|param_e_s3__AT0| Bool) (|call_synapse.xite3__AT0| Int) (|call_synapse.xite4__AT0| Int) (|call_synapse.xite15__AT0| Int) (|call_synapse.xite16__AT0| Int) (|flby__AT0| Bool)) (=> (and (state |call_First.flby__AT0| |call_First.First__AT0| |call_Sofar.flby__AT0| |call_Sofar.Sofar__AT0| |param_e_s3__AT0| |param_e_s2__AT0| |param_e_s1__AT0| |call_excludes3.excludes__AT0| |call_synapse.flby__AT0| |call_synapse.mem_init_s__AT0| |call_synapse.flby4__AT0| |call_synapse.dirty_s__AT0| |call_synapse.flby3__AT0| |call_synapse.valid_s__AT0| |call_synapse.flby2__AT0| |call_synapse.invalid_s__AT0| |call_synapse.xite17__AT0| |call_synapse.xite16__AT0| |call_synapse.xite14__AT0| |call_synapse.xite11__AT0| |call_synapse.xite10__AT0| |call_synapse.xite8__AT0| |call_synapse.xite5__AT0| |call_synapse.xite2__AT0| |call_synapse.xite4__AT0| |call_synapse.xite18__AT0| |call_synapse.xite13__AT0| |call_synapse.xite12__AT0| |call_synapse.xite7__AT0| |call_synapse.xite6__AT0| |call_synapse.xite__AT0| |env__AT0| |mem_init_s__AT0| |dirty_s__AT0| |valid_s__AT0| |invalid_s__AT0| |flby__AT0| |_OK___AT0| |param_init_invalid_s__AT0| |call_synapse.xite9__AT0| |call_synapse.garde_s3__AT0| |call_synapse.xite15__AT0| |call_synapse.garde_s2__AT0| |call_synapse.garde_s1__AT0| |call_synapse.xite3__AT0|) (let ((.def_444 (= call_synapse.invalid_s__AT0 call_synapse.xite4__AT1)))
(let ((.def_445 (or param_e_s3__AT1 .def_444)))
(let ((.def_442 (= call_synapse.xite4__AT1 call_synapse.xite3__AT1)))
(let ((.def_436 (not param_e_s3__AT1)))
(let ((.def_443 (or .def_436 .def_442)))
(let ((.def_446 (and .def_443 .def_445)))
(let ((.def_440 (= call_synapse.xite10__AT1 call_synapse.xite9__AT1)))
(let ((.def_441 (or .def_436 .def_440)))
(let ((.def_447 (and .def_441 .def_446)))
(let ((.def_438 (= call_synapse.valid_s__AT0 call_synapse.xite10__AT1)))
(let ((.def_439 (or param_e_s3__AT1 .def_438)))
(let ((.def_448 (and .def_439 .def_447)))
(let ((.def_435 (= call_synapse.xite16__AT1 call_synapse.xite15__AT1)))
(let ((.def_437 (or .def_435 .def_436)))
(let ((.def_449 (and .def_437 .def_448)))
(let ((.def_433 (= call_synapse.dirty_s__AT0 call_synapse.xite16__AT1)))
(let ((.def_434 (or param_e_s3__AT1 .def_433)))
(let ((.def_450 (and .def_434 .def_449)))
(let ((.def_291 (* (- 1) invalid_s__AT1)))
(let ((.def_292 (* (- 1) valid_s__AT1)))
(let ((.def_293 (+ .def_292 .def_291)))
(let ((.def_294 (* (- 1) dirty_s__AT1)))
(let ((.def_295 (+ .def_294 .def_293)))
(let ((.def_428 (+ invalid_s__AT0 .def_295)))
(let ((.def_429 (+ valid_s__AT0 .def_428)))
(let ((.def_430 (+ dirty_s__AT0 .def_429)))
(let ((.def_431 (= .def_430 0)))
(let ((.def_432 (= flby__AT1 .def_431)))
(let ((.def_451 (and .def_432 .def_450)))
(let ((.def_416 (= call_synapse.invalid_s__AT0 call_synapse.xite__AT1)))
(let ((.def_417 (or call_synapse.garde_s1__AT1 .def_416)))
(let ((.def_402 (* (- 1) call_synapse.invalid_s__AT0)))
(let ((.def_412 (+ .def_402 call_synapse.xite__AT1)))
(let ((.def_413 (+ call_synapse.dirty_s__AT0 .def_412)))
(let ((.def_414 (= .def_413 (- 1))))
(let ((.def_359 (not call_synapse.garde_s1__AT1)))
(let ((.def_415 (or .def_359 .def_414)))
(let ((.def_418 (and .def_415 .def_417)))
(let ((.def_403 (+ .def_402 call_synapse.xite2__AT1)))
(let ((.def_404 (* (- 1) call_synapse.valid_s__AT0)))
(let ((.def_405 (+ .def_404 .def_403)))
(let ((.def_406 (+ call_synapse.dirty_s__AT0 .def_405)))
(let ((.def_407 (= .def_406 (- 1))))
(let ((.def_350 (not call_synapse.garde_s2__AT1)))
(let ((.def_408 (or .def_350 .def_407)))
(let ((.def_419 (and .def_408 .def_418)))
(let ((.def_395 (= call_synapse.invalid_s__AT0 call_synapse.xite2__AT1)))
(let ((.def_396 (or call_synapse.garde_s2__AT1 .def_395)))
(let ((.def_420 (and .def_396 .def_419)))
(let ((.def_386 (* (- 1) call_synapse.xite3__AT1)))
(let ((.def_390 (+ call_synapse.invalid_s__AT0 .def_386)))
(let ((.def_391 (+ call_synapse.valid_s__AT0 .def_390)))
(let ((.def_392 (+ call_synapse.dirty_s__AT0 .def_391)))
(let ((.def_393 (= .def_392 1)))
(let ((.def_339 (not call_synapse.garde_s3__AT1)))
(let ((.def_394 (or .def_339 .def_393)))
(let ((.def_421 (and .def_394 .def_420)))
(let ((.def_384 (= call_synapse.invalid_s__AT0 call_synapse.xite3__AT1)))
(let ((.def_385 (or call_synapse.garde_s3__AT1 .def_384)))
(let ((.def_422 (and .def_385 .def_421)))
(let ((.def_452 (and .def_422 .def_451)))
(let ((.def_378 (= call_synapse.valid_s__AT0 call_synapse.xite7__AT1)))
(let ((.def_379 (or call_synapse.garde_s1__AT1 .def_378)))
(let ((.def_374 (* (- 1) call_synapse.xite7__AT1)))
(let ((.def_375 (+ call_synapse.valid_s__AT0 .def_374)))
(let ((.def_376 (= .def_375 (- 1))))
(let ((.def_377 (or .def_359 .def_376)))
(let ((.def_380 (and .def_377 .def_379)))
(let ((.def_372 (= call_synapse.valid_s__AT0 call_synapse.xite8__AT1)))
(let ((.def_373 (or call_synapse.garde_s2__AT1 .def_372)))
(let ((.def_381 (and .def_373 .def_380)))
(let ((.def_370 (= call_synapse.valid_s__AT0 call_synapse.xite9__AT1)))
(let ((.def_371 (or call_synapse.garde_s3__AT1 .def_370)))
(let ((.def_382 (and .def_371 .def_381)))
(let ((.def_453 (and .def_382 .def_452)))
(let ((.def_366 (= call_synapse.dirty_s__AT0 call_synapse.xite14__AT1)))
(let ((.def_367 (or call_synapse.garde_s2__AT1 .def_366)))
(let ((.def_364 (= call_synapse.dirty_s__AT0 call_synapse.xite13__AT1)))
(let ((.def_365 (or call_synapse.garde_s1__AT1 .def_364)))
(let ((.def_368 (and .def_365 .def_367)))
(let ((.def_362 (= call_synapse.dirty_s__AT0 call_synapse.xite15__AT1)))
(let ((.def_363 (or call_synapse.garde_s3__AT1 .def_362)))
(let ((.def_369 (and .def_363 .def_368)))
(let ((.def_454 (and .def_369 .def_453)))
(let ((.def_358 (= call_synapse.xite13__AT1 0)))
(let ((.def_360 (or .def_358 .def_359)))
(let ((.def_334 (<= 1 call_synapse.invalid_s__AT0)))
(let ((.def_357 (= .def_334 call_synapse.garde_s1__AT1)))
(let ((.def_361 (and .def_357 .def_360)))
(let ((.def_455 (and .def_361 .def_454)))
(let ((.def_352 (= call_synapse.xite14__AT1 1)))
(let ((.def_353 (or .def_350 .def_352)))
(let ((.def_349 (= call_synapse.xite8__AT1 0)))
(let ((.def_351 (or .def_349 .def_350)))
(let ((.def_354 (and .def_351 .def_353)))
(let ((.def_346 (<= 1 call_synapse.valid_s__AT0)))
(let ((.def_348 (= .def_346 call_synapse.garde_s2__AT1)))
(let ((.def_355 (and .def_348 .def_354)))
(let ((.def_456 (and .def_355 .def_455)))
(let ((.def_342 (= call_synapse.xite15__AT1 1)))
(let ((.def_343 (or .def_339 .def_342)))
(let ((.def_338 (= call_synapse.xite9__AT1 0)))
(let ((.def_340 (or .def_338 .def_339)))
(let ((.def_344 (and .def_340 .def_343)))
(let ((.def_336 (= .def_334 call_synapse.garde_s3__AT1)))
(let ((.def_345 (and .def_336 .def_344)))
(let ((.def_457 (and .def_345 .def_456)))
(let ((.def_333 (= call_synapse.mem_init_s__AT0 call_synapse.flby__AT1)))
(let ((.def_458 (and .def_333 .def_457)))
(let ((.def_332 (= call_synapse.flby2__AT1 call_synapse.xite6__AT1)))
(let ((.def_459 (and .def_332 .def_458)))
(let ((.def_331 (= call_synapse.flby3__AT1 call_synapse.xite12__AT1)))
(let ((.def_460 (and .def_331 .def_459)))
(let ((.def_330 (= call_synapse.flby4__AT1 call_synapse.xite18__AT1)))
(let ((.def_461 (and .def_330 .def_460)))
(let ((.def_326 (<= 0 param_init_invalid_s__AT1)))
(let ((.def_327 (and call_excludes3.excludes__AT1 .def_326)))
(let ((.def_328 (and call_Sofar.Sofar__AT0 .def_327)))
(let ((.def_329 (= call_Sofar.flby__AT1 .def_328)))
(let ((.def_462 (and .def_329 .def_461)))
(let ((.def_324 (= call_First.First__AT0 call_First.flby__AT1)))
(let ((.def_463 (and .def_324 .def_462)))
(let ((.def_305 (not env__AT1)))
(let ((.def_301 (<= 1 dirty_s__AT1)))
(let ((.def_302 (not .def_301)))
(let ((.def_299 (<= 1 valid_s__AT1)))
(let ((.def_300 (not .def_299)))
(let ((.def_303 (or .def_300 .def_302)))
(let ((.def_296 (+ call_First.First__AT1 .def_295)))
(let ((.def_297 (= .def_296 0)))
(let ((.def_283 (<= 2 dirty_s__AT1)))
(let ((.def_284 (not .def_283)))
(let ((.def_286 (and .def_284 flby__AT1)))
(let ((.def_298 (and .def_286 .def_297)))
(let ((.def_304 (and .def_298 .def_303)))
(let ((.def_306 (or .def_304 .def_305)))
(let ((.def_308 (= .def_306 _OK___AT1)))
(let ((.def_282 (= call_synapse.invalid_s__AT1 invalid_s__AT1)))
(let ((.def_309 (and .def_282 .def_308)))
(let ((.def_280 (= call_synapse.valid_s__AT1 valid_s__AT1)))
(let ((.def_310 (and .def_280 .def_309)))
(let ((.def_278 (= call_synapse.dirty_s__AT1 dirty_s__AT1)))
(let ((.def_311 (and .def_278 .def_310)))
(let ((.def_276 (= call_synapse.mem_init_s__AT1 mem_init_s__AT1)))
(let ((.def_312 (and .def_276 .def_311)))
(let ((.def_274 (= call_Sofar.Sofar__AT1 env__AT1)))
(let ((.def_313 (and .def_274 .def_312)))
(let ((.def_266 (= call_synapse.xite5__AT1 call_synapse.xite6__AT1)))
(let ((.def_267 (or param_e_s1__AT1 .def_266)))
(let ((.def_264 (= call_synapse.xite__AT1 call_synapse.xite6__AT1)))
(let ((.def_254 (not param_e_s1__AT1)))
(let ((.def_265 (or .def_254 .def_264)))
(let ((.def_268 (and .def_265 .def_267)))
(let ((.def_260 (= call_synapse.xite12__AT1 call_synapse.xite7__AT1)))
(let ((.def_261 (or .def_254 .def_260)))
(let ((.def_269 (and .def_261 .def_268)))
(let ((.def_257 (= call_synapse.xite11__AT1 call_synapse.xite12__AT1)))
(let ((.def_258 (or param_e_s1__AT1 .def_257)))
(let ((.def_270 (and .def_258 .def_269)))
(let ((.def_253 (= call_synapse.xite18__AT1 call_synapse.xite13__AT1)))
(let ((.def_255 (or .def_253 .def_254)))
(let ((.def_271 (and .def_255 .def_270)))
(let ((.def_250 (= call_synapse.xite17__AT1 call_synapse.xite18__AT1)))
(let ((.def_251 (or param_e_s1__AT1 .def_250)))
(let ((.def_272 (and .def_251 .def_271)))
(let ((.def_314 (and .def_272 .def_313)))
(let ((.def_242 (= call_synapse.xite5__AT1 call_synapse.xite4__AT1)))
(let ((.def_243 (or param_e_s2__AT1 .def_242)))
(let ((.def_239 (= call_synapse.xite2__AT1 call_synapse.xite5__AT1)))
(let ((.def_228 (not param_e_s2__AT1)))
(let ((.def_240 (or .def_228 .def_239)))
(let ((.def_244 (and .def_240 .def_243)))
(let ((.def_235 (= call_synapse.xite11__AT1 call_synapse.xite8__AT1)))
(let ((.def_236 (or .def_228 .def_235)))
(let ((.def_245 (and .def_236 .def_244)))
(let ((.def_232 (= call_synapse.xite10__AT1 call_synapse.xite11__AT1)))
(let ((.def_233 (or param_e_s2__AT1 .def_232)))
(let ((.def_246 (and .def_233 .def_245)))
(let ((.def_227 (= call_synapse.xite17__AT1 call_synapse.xite14__AT1)))
(let ((.def_229 (or .def_227 .def_228)))
(let ((.def_247 (and .def_229 .def_246)))
(let ((.def_224 (= call_synapse.xite16__AT1 call_synapse.xite17__AT1)))
(let ((.def_225 (or param_e_s2__AT1 .def_224)))
(let ((.def_248 (and .def_225 .def_247)))
(let ((.def_315 (and .def_248 .def_314)))
(let ((.def_221 (= call_synapse.invalid_s__AT1 call_synapse.flby2__AT1)))
(let ((.def_316 (and .def_221 .def_315)))
(let ((.def_218 (= call_synapse.valid_s__AT1 call_synapse.flby3__AT1)))
(let ((.def_317 (and .def_218 .def_316)))
(let ((.def_215 (= call_synapse.dirty_s__AT1 call_synapse.flby4__AT1)))
(let ((.def_318 (and .def_215 .def_317)))
(let ((.def_212 (= call_synapse.mem_init_s__AT1 call_synapse.flby__AT1)))
(let ((.def_319 (and .def_212 .def_318)))
(let ((.def_204 (and param_e_s2__AT1 param_e_s1__AT1)))
(let ((.def_203 (and param_e_s3__AT1 param_e_s1__AT1)))
(let ((.def_205 (or .def_203 .def_204)))
(let ((.def_201 (and param_e_s2__AT1 param_e_s3__AT1)))
(let ((.def_206 (or .def_201 .def_205)))
(let ((.def_207 (not .def_206)))
(let ((.def_209 (= .def_207 call_excludes3.excludes__AT1)))
(let ((.def_320 (and .def_209 .def_319)))
(let ((.def_198 (= call_Sofar.Sofar__AT1 call_Sofar.flby__AT1)))
(let ((.def_321 (and .def_198 .def_320)))
(let ((.def_195 (= call_First.First__AT1 call_First.flby__AT1)))
(let ((.def_322 (and .def_195 .def_321)))
(let ((.def_125 (not env__AT0)))
(let ((.def_121 (<= 1 dirty_s__AT0)))
(let ((.def_122 (not .def_121)))
(let ((.def_119 (<= 1 valid_s__AT0)))
(let ((.def_120 (not .def_119)))
(let ((.def_123 (or .def_120 .def_122)))
(let ((.def_111 (* (- 1) invalid_s__AT0)))
(let ((.def_112 (* (- 1) valid_s__AT0)))
(let ((.def_113 (+ .def_112 .def_111)))
(let ((.def_114 (* (- 1) dirty_s__AT0)))
(let ((.def_115 (+ .def_114 .def_113)))
(let ((.def_116 (+ call_First.First__AT0 .def_115)))
(let ((.def_117 (= .def_116 0)))
(let ((.def_100 (<= 2 dirty_s__AT0)))
(let ((.def_101 (not .def_100)))
(let ((.def_103 (and .def_101 flby__AT0)))
(let ((.def_118 (and .def_103 .def_117)))
(let ((.def_124 (and .def_118 .def_123)))
(let ((.def_126 (or .def_124 .def_125)))
(let ((.def_128 (= .def_126 _OK___AT0)))
(let ((.def_98 (= call_synapse.invalid_s__AT0 invalid_s__AT0)))
(let ((.def_129 (and .def_98 .def_128)))
(let ((.def_96 (= call_synapse.valid_s__AT0 valid_s__AT0)))
(let ((.def_130 (and .def_96 .def_129)))
(let ((.def_94 (= call_synapse.dirty_s__AT0 dirty_s__AT0)))
(let ((.def_131 (and .def_94 .def_130)))
(let ((.def_92 (= call_synapse.mem_init_s__AT0 mem_init_s__AT0)))
(let ((.def_132 (and .def_92 .def_131)))
(let ((.def_90 (= call_Sofar.Sofar__AT0 env__AT0)))
(let ((.def_133 (and .def_90 .def_132)))
(let ((.def_82 (= call_synapse.xite5__AT0 call_synapse.xite6__AT0)))
(let ((.def_83 (or param_e_s1__AT0 .def_82)))
(let ((.def_80 (= call_synapse.xite6__AT0 call_synapse.xite__AT0)))
(let ((.def_70 (not param_e_s1__AT0)))
(let ((.def_81 (or .def_70 .def_80)))
(let ((.def_84 (and .def_81 .def_83)))
(let ((.def_76 (= call_synapse.xite12__AT0 call_synapse.xite7__AT0)))
(let ((.def_77 (or .def_70 .def_76)))
(let ((.def_85 (and .def_77 .def_84)))
(let ((.def_73 (= call_synapse.xite11__AT0 call_synapse.xite12__AT0)))
(let ((.def_74 (or param_e_s1__AT0 .def_73)))
(let ((.def_86 (and .def_74 .def_85)))
(let ((.def_69 (= call_synapse.xite18__AT0 call_synapse.xite13__AT0)))
(let ((.def_71 (or .def_69 .def_70)))
(let ((.def_87 (and .def_71 .def_86)))
(let ((.def_66 (= call_synapse.xite17__AT0 call_synapse.xite18__AT0)))
(let ((.def_67 (or param_e_s1__AT0 .def_66)))
(let ((.def_88 (and .def_67 .def_87)))
(let ((.def_134 (and .def_88 .def_133)))
(let ((.def_58 (= call_synapse.xite5__AT0 call_synapse.xite4__AT0)))
(let ((.def_59 (or param_e_s2__AT0 .def_58)))
(let ((.def_55 (= call_synapse.xite5__AT0 call_synapse.xite2__AT0)))
(let ((.def_44 (not param_e_s2__AT0)))
(let ((.def_56 (or .def_44 .def_55)))
(let ((.def_60 (and .def_56 .def_59)))
(let ((.def_51 (= call_synapse.xite11__AT0 call_synapse.xite8__AT0)))
(let ((.def_52 (or .def_44 .def_51)))
(let ((.def_61 (and .def_52 .def_60)))
(let ((.def_48 (= call_synapse.xite11__AT0 call_synapse.xite10__AT0)))
(let ((.def_49 (or param_e_s2__AT0 .def_48)))
(let ((.def_62 (and .def_49 .def_61)))
(let ((.def_43 (= call_synapse.xite17__AT0 call_synapse.xite14__AT0)))
(let ((.def_45 (or .def_43 .def_44)))
(let ((.def_63 (and .def_45 .def_62)))
(let ((.def_40 (= call_synapse.xite17__AT0 call_synapse.xite16__AT0)))
(let ((.def_41 (or param_e_s2__AT0 .def_40)))
(let ((.def_64 (and .def_41 .def_63)))
(let ((.def_135 (and .def_64 .def_134)))
(let ((.def_37 (= call_synapse.flby2__AT0 call_synapse.invalid_s__AT0)))
(let ((.def_136 (and .def_37 .def_135)))
(let ((.def_34 (= call_synapse.flby3__AT0 call_synapse.valid_s__AT0)))
(let ((.def_137 (and .def_34 .def_136)))
(let ((.def_31 (= call_synapse.flby4__AT0 call_synapse.dirty_s__AT0)))
(let ((.def_138 (and .def_31 .def_137)))
(let ((.def_28 (= call_synapse.flby__AT0 call_synapse.mem_init_s__AT0)))
(let ((.def_139 (and .def_28 .def_138)))
(let ((.def_20 (and param_e_s2__AT0 param_e_s1__AT0)))
(let ((.def_19 (and param_e_s3__AT0 param_e_s1__AT0)))
(let ((.def_21 (or .def_19 .def_20)))
(let ((.def_17 (and param_e_s3__AT0 param_e_s2__AT0)))
(let ((.def_22 (or .def_17 .def_21)))
(let ((.def_23 (not .def_22)))
(let ((.def_25 (= .def_23 call_excludes3.excludes__AT0)))
(let ((.def_140 (and .def_25 .def_139)))
(let ((.def_14 (= call_Sofar.flby__AT0 call_Sofar.Sofar__AT0)))
(let ((.def_141 (and .def_14 .def_140)))
(let ((.def_11 (= call_First.flby__AT0 call_First.First__AT0)))
(let ((.def_142 (and .def_11 .def_141)))
(let ((.def_323 (and .def_142 .def_322)))
(let ((.def_464 (and .def_323 .def_463)))
.def_464)))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))) (state |call_First.flby__AT1| |call_First.First__AT1| |call_Sofar.flby__AT1| |call_Sofar.Sofar__AT1| |param_e_s3__AT1| |param_e_s2__AT1| |param_e_s1__AT1| |call_excludes3.excludes__AT1| |call_synapse.flby__AT1| |call_synapse.mem_init_s__AT1| |call_synapse.flby4__AT1| |call_synapse.dirty_s__AT1| |call_synapse.flby3__AT1| |call_synapse.valid_s__AT1| |call_synapse.flby2__AT1| |call_synapse.invalid_s__AT1| |call_synapse.xite17__AT1| |call_synapse.xite16__AT1| |call_synapse.xite14__AT1| |call_synapse.xite11__AT1| |call_synapse.xite10__AT1| |call_synapse.xite8__AT1| |call_synapse.xite5__AT1| |call_synapse.xite2__AT1| |call_synapse.xite4__AT1| |call_synapse.xite18__AT1| |call_synapse.xite13__AT1| |call_synapse.xite12__AT1| |call_synapse.xite7__AT1| |call_synapse.xite6__AT1| |call_synapse.xite__AT1| |env__AT1| |mem_init_s__AT1| |dirty_s__AT1| |valid_s__AT1| |invalid_s__AT1| |flby__AT1| |_OK___AT1| |param_init_invalid_s__AT1| |call_synapse.xite9__AT1| |call_synapse.garde_s3__AT1| |call_synapse.xite15__AT1| |call_synapse.garde_s2__AT1| |call_synapse.garde_s1__AT1| |call_synapse.xite3__AT1|))))
(assert (forall ((|call_synapse.garde_s1__AT0| Bool) (|call_synapse.xite13__AT0| Int) (|call_synapse.xite14__AT0| Int) (|call_synapse.garde_s2__AT0| Bool) (|call_synapse.xite8__AT0| Int) (|call_synapse.garde_s3__AT0| Bool) (|param_init_invalid_s__AT0| Int) (|call_synapse.flby__AT0| Int) (|call_synapse.flby2__AT0| Int) (|call_synapse.mem_init_s__AT0| Int) (|call_synapse.flby3__AT0| Int) (|call_synapse.flby4__AT0| Int) (|call_excludes3.excludes__AT0| Bool) (|call_Sofar.flby__AT0| Bool) (|call_First.flby__AT0| Int) (|_OK___AT0| Bool) (|env__AT0| Bool) (|dirty_s__AT0| Int) (|valid_s__AT0| Int) (|invalid_s__AT0| Int) (|call_synapse.dirty_s__AT0| Int) (|mem_init_s__AT0| Int) (|call_Sofar.Sofar__AT0| Bool) (|call_synapse.invalid_s__AT0| Int) (|call_synapse.xite6__AT0| Int) (|call_synapse.valid_s__AT0| Int) (|call_synapse.xite5__AT0| Int) (|call_First.First__AT0| Int) (|param_e_s1__AT0| Bool) (|call_synapse.xite__AT0| Int) (|call_synapse.xite7__AT0| Int) (|call_synapse.xite12__AT0| Int) (|call_synapse.xite11__AT0| Int) (|call_synapse.xite18__AT0| Int) (|call_synapse.xite17__AT0| Int) (|param_e_s2__AT0| Bool) (|call_synapse.xite2__AT0| Int) (|call_synapse.xite4__AT1| Int) (|param_e_s3__AT1| Bool) (|call_synapse.xite3__AT1| Int) (|call_synapse.xite9__AT1| Int) (|call_synapse.xite10__AT1| Int) (|call_synapse.xite15__AT1| Int) (|call_synapse.xite16__AT1| Int) (|invalid_s__AT1| Int) (|valid_s__AT1| Int) (|dirty_s__AT1| Int) (|flby__AT1| Bool) (|call_synapse.xite__AT1| Int) (|call_synapse.garde_s1__AT1| Bool) (|call_synapse.xite2__AT1| Int) (|call_synapse.garde_s2__AT1| Bool) (|call_synapse.garde_s3__AT1| Bool) (|call_synapse.xite7__AT1| Int) (|call_synapse.xite8__AT1| Int) (|call_synapse.xite14__AT1| Int) (|call_synapse.xite13__AT1| Int) (|call_synapse.flby__AT1| Int) (|call_synapse.xite6__AT1| Int) (|call_synapse.flby2__AT1| Int) (|call_synapse.xite12__AT1| Int) (|call_synapse.flby3__AT1| Int) (|call_synapse.xite18__AT1| Int) (|call_synapse.flby4__AT1| Int) (|param_init_invalid_s__AT1| Int) (|call_excludes3.excludes__AT1| Bool) (|call_Sofar.flby__AT1| Bool) (|call_First.flby__AT1| Int) (|_OK___AT1| Bool) (|env__AT1| Bool) (|call_First.First__AT1| Int) (|call_synapse.invalid_s__AT1| Int) (|call_synapse.valid_s__AT1| Int) (|call_synapse.dirty_s__AT1| Int) (|mem_init_s__AT1| Int) (|call_synapse.mem_init_s__AT1| Int) (|call_Sofar.Sofar__AT1| Bool) (|call_synapse.xite5__AT1| Int) (|param_e_s1__AT1| Bool) (|call_synapse.xite11__AT1| Int) (|call_synapse.xite17__AT1| Int) (|param_e_s2__AT1| Bool) (|call_synapse.xite9__AT0| Int) (|call_synapse.xite10__AT0| Int) (|param_e_s3__AT0| Bool) (|call_synapse.xite3__AT0| Int) (|call_synapse.xite4__AT0| Int) (|call_synapse.xite15__AT0| Int) (|call_synapse.xite16__AT0| Int) (|flby__AT0| Bool)) (=> (state |call_First.flby__AT0| |call_First.First__AT0| |call_Sofar.flby__AT0| |call_Sofar.Sofar__AT0| |param_e_s3__AT0| |param_e_s2__AT0| |param_e_s1__AT0| |call_excludes3.excludes__AT0| |call_synapse.flby__AT0| |call_synapse.mem_init_s__AT0| |call_synapse.flby4__AT0| |call_synapse.dirty_s__AT0| |call_synapse.flby3__AT0| |call_synapse.valid_s__AT0| |call_synapse.flby2__AT0| |call_synapse.invalid_s__AT0| |call_synapse.xite17__AT0| |call_synapse.xite16__AT0| |call_synapse.xite14__AT0| |call_synapse.xite11__AT0| |call_synapse.xite10__AT0| |call_synapse.xite8__AT0| |call_synapse.xite5__AT0| |call_synapse.xite2__AT0| |call_synapse.xite4__AT0| |call_synapse.xite18__AT0| |call_synapse.xite13__AT0| |call_synapse.xite12__AT0| |call_synapse.xite7__AT0| |call_synapse.xite6__AT0| |call_synapse.xite__AT0| |env__AT0| |mem_init_s__AT0| |dirty_s__AT0| |valid_s__AT0| |invalid_s__AT0| |flby__AT0| |_OK___AT0| |param_init_invalid_s__AT0| |call_synapse.xite9__AT0| |call_synapse.garde_s3__AT0| |call_synapse.xite15__AT0| |call_synapse.garde_s2__AT0| |call_synapse.garde_s1__AT0| |call_synapse.xite3__AT0|) _OK___AT0)))
(check-sat)
